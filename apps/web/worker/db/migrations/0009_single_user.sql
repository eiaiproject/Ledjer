-- Migration: 0009_single_user.sql (phased for D1 defer)
-- Fase 1 (de-multi-tenant): single-user, phased rebuild to avoid FK violation on D1.
-- See ADR 0002. Phased: create all _v2, then drop originals child-first, then rename parent-first.
...
-- PHASE 1: create all _v2 and backfill (no drops yet)
...
CREATE TABLE IF NOT EXISTS transactions_v2 (
  ...
  updated_at INTEGER NOT NULL,
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
  FOREIGN KEY (cash_account_id) REFERENCES accounts_v2(id),
  FOREIGN KEY (counter_account_id) REFERENCES accounts_v2(id)
);
...
CREATE TABLE IF NOT EXISTS journal_lines_v2 (
  ...
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
  FOREIGN KEY (journal_entry_id) REFERENCES journal_entries_v2(id) ON DELETE CASCADE,
...
-- PHASE 2: drop originals child-first (most dependent first)
...
-- PHASE 3: rename _v2 parent-first
...
-- PHASE 4: drop multi-tenant tables
