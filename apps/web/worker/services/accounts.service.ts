...
const accountColumns = "id, user_id, code, name, account_class, account_subtype, account_kind, is_system, is_active, created_at, updated_at";
...
export async function getAccount(
  ...
  return queryFirst<AccountRow>(
    db,
    `SELECT ${accountColumns} FROM accounts WHERE id = ? AND user_id = ?`,
    [accountId, userId],
  );
}
...
/** Akun sistem bertanda (Persediaan/HPP) milik user, aktif. */
export async function getAccountByKind(
  ...
  return queryFirst<AccountRow>(
    db,
    `SELECT ${accountColumns} FROM accounts WHERE user_id = ? AND account_kind = ? AND is_active = 1`,
    [userId, kind],
  );
}
 ...
 * unique within the user's book (optionally excluding the account being
...
async function assertValidAccountName(
  ...
  const existing = await queryFirst<{ id: string }>(
    ...
    excludeAccountId ? [userId, name, excludeAccountId] : [userId, name],
  );
  if (existing) throw badRequest("account_name_taken", "Nama akun sudah dipakai.");
  return name;
}
...
export async function createCashBankAccount(
  ...
  const name = await assertValidAccountName(db, userId, input.name);

  const code = await nextCashBankCode(db, userId);
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const attemptCode = attempt === 0 ? code : await nextCashBankCode(db, userId);
    ...
    try {
      await execute(
        ...
        [attemptId, userId, attemptCode, name, input.subtype, current, current],
      );
      await logAuthEvent(db, userId, "account_created", { accountId: attemptId, code: attemptCode, name, subtype: input.subtype });
      const account = await getAccount(db, userId, attemptId);
...
export async function nextAccountCode(
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    const existing = await queryFirst<{ id: string }>(
      db,
      "SELECT id FROM accounts WHERE user_id = ? AND code = ?",
      [userId, String(next)],
    );
...
export async function createAccount(
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const attemptCode = await nextAccountCode(db, userId, input.accountClass);
    const attemptId = attempt === 0 ? accountId : crypto.randomUUID();
    try {
      await execute(
        db,
        ...
        [attemptId, userId, attemptCode, name, input.accountClass, current, current],
      );
      await logAuthEvent(db, userId, "account_created", { accountId: attemptId, code: attemptCode, name });
      const account = await getAccount(db, userId, attemptId);
      if (!account) throw badRequest("account_create_failed", "Gagal membuat akun.");
      return account;
    } catch (err) {
...
export async function patchAccount(
  ...
  if (input.name !== undefined) {
    const name = await assertValidAccountName(db, userId, input.name, accountId);
    updates.push("name = ?");
    values.push(name);
  }
  ...
  if (input.isActive !== undefined) {
    ...
    if (!input.isActive && (await accountIsUsed(db, userId, accountId))) {
  ...
  values.push(current, accountId, userId);
  await execute(
    db,
    `UPDATE accounts SET ${updates.join(", ")}, updated_at = ? WHERE id = ? AND user_id = ?`,
    values,
  );
  await logAuthEvent(db, userId, "account_updated", { accountId, ...input });
...
export async function nextCashBankCode(db: D1Database, userId: string): Promise<string> {
  const row = await queryFirst<{ max_code: number | null }>(
    db,
    `SELECT MAX(CAST(code AS INTEGER)) AS max_code
     FROM accounts
     WHERE user_id = ? AND account_subtype IS NOT NULL`,
    [userId],
  );
  // Lewati kode yang sudah dipakai akun lain (mis. Persediaan 1130) agar
  // tidak menabrak UNIQUE(user_id, code) pada akun kas/bank baru.
...
export async function accountIsUsed(
  ...
  const row = await queryFirst<{ c: number }>(
    ...
    `SELECT COUNT(*) AS c FROM transactions
     WHERE user_id = ? AND status = 'posted' AND (cash_account_id = ? OR counter_account_id = ?)`,
...
export async function balancesByAccount(
  ...
  const rows = await queryAll<{ account_id: string; debit: number; credit: number }>(
    ...
    `SELECT jl.account_id, SUM(jl.debit_idr) AS debit, SUM(jl.credit_idr) AS credit
     ...
     WHERE jl.user_id = ? AND t.status = 'posted'
...
export async function accountBalance(
  ...
  const row = await queryFirst<{ debit: number; credit: number }>(
    ...
    `SELECT SUM(jl.debit_idr) AS debit, SUM(jl.credit_idr) AS credit
     ...
     JOIN journal_entries je ON je.id = jl.journal_entry_id
     JOIN transactions t ON t.id = je.transaction_id
     WHERE jl.user_id = ? AND jl.account_id = ? AND t.status = 'posted'`,
    [userId, accountId],
  );
