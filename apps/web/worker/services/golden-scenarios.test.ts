 ...
 *   - Cross-book isolation of reports
 */
...
describe("Golden Accounting Scenarios", () => {
  it("G1: posting a cash_in keeps every journal balanced and moves balances", async () => {
    const db = freshDb();
    const before = await listAccounts(db, OWNER_A);
  ...
  it("G3: voiding a cash_out restores the balance and removes it from reports", async () => {
    const db = freshDb();
    const before = await getBalanceSheet(db, OWNER_A, "2026-06-30");
    const cashBefore = before.assets.find((a) => a.code === "1110")!.amount;

    await voidTransaction(db, OWNER_A, FIXTURE_IDS.transactions.cashOutA, {});
  ...
  it("G4: balance sheet equation holds and reports never leak across books", async () => {
    const db = freshDb();
    const bsA = await getBalanceSheet(db, OWNER_A, "2026-06-30");
    const bsB = await getBalanceSheet(db, OWNER_B, "2026-06-30");
    ...
    const plB = await getProfitLoss(db, OWNER_B, "2026-06-01", "2026-06-30");
