...
describe("listTransactions (real SQLite)", () => {
  ...
  beforeAll(() => {
    ...
    db.exec(`INSERT INTO accounts (id, user_id, code, name, account_class, account_subtype)
             VALUES ('acct-cash', '${USER}', '1110', 'Kas', 'asset', 'cash')`);
    db.exec(`INSERT INTO accounts (id, user_id, code, name, account_class, account_subtype)
             VALUES ('acct-rev', '${USER}', '4110', 'Pendapatan Usaha', 'income', NULL)`);

    return Promise.all([
      // Posted cash_in - must be visible.
      insertTransaction(db, {
        id: "txn-cashin", user_id: USER, transaction_number: "TRX-20260815-AB12",
      ...
      insertTransaction(db, {
        id: "txn-voided", user_id: USER, transaction_number: "TRX-20260816-CD34",
      ...
      insertTransaction(db, {
        id: "txn-cashout", user_id: USER, transaction_number: "TRX-20260817-EF56",
  ...
  it("lists every transaction in order - newest date first", async () => {
    const rows = await listTransactions(db as unknown as D1Database, USER, {});
  ...
  it("filters by status and search", async () => {
    const voided = await listTransactions(db as unknown as D1Database, USER, { status: "voided" });
    expect(voided).toHaveLength(1);

    const search = await listTransactions(db as unknown as D1Database, USER, {
      search: "listrik",
    });
  ...
  it("isolates by user", async () => {
    const rows = await listTransactions(db as unknown as D1Database, "other-user", {});
