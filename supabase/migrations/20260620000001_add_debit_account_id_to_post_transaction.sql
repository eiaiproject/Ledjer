-- Migration: Add p_debit_account_id to post_transaction
-- Date: 2026-06-20
-- Purpose:
--   Allow frontend to explicitly specify the debit account ID for
--   cash_purchase, credit_purchase, and expense_payment transactions.
--   This replaces the fragile ILIKE lookup on p_category_name.
--
--   When p_debit_account_id is provided:
--   - cash_purchase/credit_purchase (no product): use it directly
--   - expense_payment: use it directly
--   - cash_purchase/credit_purchase (with product): still debits Persediaan (1300)
--
...
BEGIN
  ...
  -- Validate transaction_date >= books_start_date
  ...
  -- Validate cash_account_id is an asset account
  ...
  -- Enforce party_id for credit transactions
  ...
  -- Tighten simple_adjustment validation
  ...
  CASE p_transaction_type
    ...
    WHEN 'cash_purchase' THEN
      IF p_product_id IS NOT NULL THEN
        -- Purchase for inventory: always debit Persediaan
        SELECT id INTO v_debit_account_id
      ...
      ELSIF p_debit_account_id IS NOT NULL THEN
        -- Use explicitly provided account ID (from CoA dropdown)
        v_debit_account_id := p_debit_account_id;
        SELECT name INTO v_debit_account_name
        FROM accounts WHERE id = p_debit_account_id AND organization_id = p_organization_id;
      ELSE
        -- Fallback: lookup by category name (legacy behavior)
        SELECT id INTO v_debit_account_id
        FROM accounts
        WHERE organization_id = p_organization_id
