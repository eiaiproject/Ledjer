-- Fix post_transaction function overload ambiguity
-- Drop ALL existing versions and create a single canonical function
-- that uses correct schema (current_plan), correct account lookups,
-- correct table names (stock_movements), and record_stock_movement.
--
...
BEGIN
  ...
  -- Use current_plan (not plan) and get_monthly_transaction_count
  ...
  -- Uses dynamic lookups that match the default CoA (1200 Piutang, 2100 Utang, 6110+ expense)
  CASE p_transaction_type
    ...
    WHEN 'cash_purchase' THEN
      ...
      ELSE
        -- Use p_debit_account_id from frontend if provided, otherwise fallback
        IF p_debit_account_id IS NOT NULL THEN
          v_debit_account_id := p_debit_account_id;
          SELECT name INTO v_debit_account_name
          FROM public.accounts WHERE id = p_debit_account_id AND organization_id = p_organization_id;
        ELSE
    ...
    WHEN 'expense_payment' THEN
      -- Use p_debit_account_id from frontend if provided, otherwise lookup by name/code
      IF p_debit_account_id IS NOT NULL THEN
        v_debit_account_id := p_debit_account_id;
        SELECT name INTO v_debit_account_name
        FROM public.accounts WHERE id = p_debit_account_id AND organization_id = p_organization_id;
  ...
  -- Handle product stock via record_stock_movement (uses stock_movements table)
  IF p_product_id IS NOT NULL THEN
...
GRANT EXECUTE ON FUNCTION public.post_transaction(
  UUID, DATE, TEXT, NUMERIC, UUID, TEXT, UUID, UUID, TEXT, NUMERIC, DATE, TEXT, TEXT, UUID, NUMERIC, NUMERIC, UUID
) TO authenticated;
...
BEGIN
  ...
  IF func_count != 1 THEN
    RAISE EXCEPTION 'Expected 1 post_transaction function, found %', func_count;
  END IF;
