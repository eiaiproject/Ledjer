...
BEGIN
  ...
  -- Only owner can post opening balances
  SELECT role::TEXT
  INTO v_role
  ...
  -- (has at least one non-opening transaction)
  SELECT EXISTS (
    SELECT 1
  ...
  ) INTO v_normal_txn_count;

  IF v_normal_txn_count > 0 THEN
