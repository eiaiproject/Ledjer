-- P0.1: Drop ALL post_transaction overloads, recreate single canonical function.
...
-- Also drop any other arity we might have missed
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    SELECT oid, proargtypes::text[] AS args
    FROM pg_proc
    WHERE proname = 'post_transaction'
      AND pronamespace = 'public'::regnamespace
  LOOP
    EXECUTE format('DROP FUNCTION public.post_transaction(%s) CASCADE',
                   pg_catalog.pg_function_arguments(r.oid));
  END LOOP;
END $$;

-- ── 2. Helper: get_account_by_code ──────────────────────────────────
...
REVOKE EXECUTE ON FUNCTION public.get_account_by_code(UUID, INTEGER) FROM PUBLIC, anon;

-- ── 3. Canonical post_transaction ───────────────────────────────────
...
BEGIN
  ...
  IF p_product_id IS NOT NULL THEN
    ...
    -- P0.1: Validate COGS/inventory accounts for product sales BEFORE posting anything
  ...
  IF p_product_id IS NOT NULL THEN
    IF p_transaction_type IN ('cash_purchase', 'credit_purchase') THEN
      ...
      PERFORM public.record_stock_movement(
        ...
        v_transaction_id,   -- ← uses v_transaction_id, NOT v_txn_id
        p_description
      );
    ...
    ELSIF p_transaction_type IN ('cash_sale', 'credit_sale') THEN
      ...
      IF v_cogs_amount > 0 THEN
        -- COGS account 5100 + Inventory account 1300 already validated above
