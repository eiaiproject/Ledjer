...
--
-- FIX 1: Replace all v_cash_account_id references with p_cash_account_id
--         (the function parameter). No new variable declaration needed.
--
-- FIX 2: Add cash account validation for credit_sale/credit_purchase partial
--         payments. The original cash account validation block only covered
--         explicit cash transaction types, leaving partial credit payments
--         without org-scoped cash account validation.
-- =============================================================================

-- Drop the existing function and recreate with the fix
DROP FUNCTION IF EXISTS public.post_transaction(
  UUID, DATE, TEXT, NUMERIC, UUID, TEXT, UUID, UUID,
  TEXT, NUMERIC, DATE, TEXT, TEXT, UUID, NUMERIC, NUMERIC, UUID, UUID, TEXT
);
...
BEGIN
  ...
  -- ── FIX #3: Validate cash account for partial credit payments ──
  -- When payment_status = 'partial', p_cash_account_id is used in journal_lines
  -- but the above block doesn't cover credit_sale/credit_purchase. Validate here.
  IF p_transaction_type IN ('credit_sale', 'credit_purchase')
     AND p_payment_status = 'partial' THEN
    IF p_cash_account_id IS NULL THEN
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Akun kas/bank tidak ditemukan atau tidak termasuk dalam organisasi ini';
    END IF;
    IF v_account_type != 'asset' OR v_is_cash_account != true THEN
      RAISE EXCEPTION 'Akun kas/bank tidak valid untuk pembayaran sebagian';
    END IF;
  END IF;
...
-- Re-grant to service_role for admin operations
