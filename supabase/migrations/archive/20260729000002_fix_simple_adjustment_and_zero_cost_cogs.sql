...
--  to the same organization as the journal line.
-- ═══════════════════════════════════════════════════════════════════
CREATE OR REPLACE FUNCTION public.enforce_journal_line_org_match()
RETURNS TRIGGER AS $$
DECLARE
  v_acct_org UUID;
BEGIN
...
DROP TRIGGER IF EXISTS trg_enforce_journal_line_org_match ON public.journal_lines;
CREATE TRIGGER trg_enforce_journal_line_org_match
  BEFORE INSERT OR UPDATE ON public.journal_lines
  FOR EACH ROW
  EXECUTE FUNCTION public.enforce_journal_line_org_match();
